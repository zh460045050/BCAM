...
if __name__ == '__main__':
    ...
    parser.add_argument("--cam_network", default="net.resnet50_bcam", type=str)
    ...
    ##BCAM
    parser.add_argument("--num_head", default=20, type=int)
    parser.add_argument("--rate_fb", default=1, type=float)
    parser.add_argument("--rate_bf", default=1, type=float)
    parser.add_argument("--rate_bb", default=1, type=float)
    ...
    # Output Path
    parser.add_argument("--check_name", default="bcam", type=str)
    ...
    args.cam_out_dir = os.path.join("results", args.check_name, args.cam_out_dir)
    args.ir_label_out_dir = os.path.join("results", args.check_name, args.ir_label_out_dir)
    args.sem_seg_out_dir = os.path.join("results", args.check_name, args.sem_seg_out_dir)
    args.ins_seg_out_dir = os.path.join("results", args.check_name, args.ins_seg_out_dir)
    args.log_name = os.path.join("results", args.check_name, args.log_name)
    args.cam_weights_name = os.path.join("results", args.check_name, args.cam_weights_name)
    args.irn_weights_name = os.path.join("results", args.check_name, args.irn_weights_name)
    

    os.makedirs("/".join(args.cam_weights_name.split('/')[:-1]), exist_ok=True)
    ...
    if args.train_cam_pass is True:
        import step.train_bcam

        timer = pyutils.Timer('step.train_cam:')
        step.train_bcam.run(args)

    if args.make_cam_pass is True:
        import step.make_bcamV1

        timer = pyutils.Timer('step.make_cam:')
        step.make_bcamV1.run(args)
